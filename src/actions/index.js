...
export const toggleLayer = (data, visible) => {
  ...
  if (layer === "background") {
    if (sublayer === "color") {
      data.scene.background = visible
        ? new THREE.Color(data.backgroundColor)
        : null;
...
export const showCoords = (x, y, z) => {
  return (dispatch, getState) => {
    ...
    document.getElementById("coords").innerHTML = `${(
      x * diffX +
      coords.min.x
    ).toFixed(2)}, ${(y * diffY + coords.min.y).toFixed(2)},${(
      coords.min.z + z
    ).toFixed(2)}`;
...
export const addModel = (options, props) => {
  ...
  if (theVector.name.toLowerCase().includes("anime")) {
    let mesh;
    if (
      d.name.toLowerCase().includes("glb") ||
      d.name.toLowerCase().includes("gltf")
    ) {
  ...
  newLayers.tree[1].children[3].children.push({
    key: `models-${group.id}`,
    title: d.name + "_" + theVector.name,
  });
...
export const loadDemo = function (props, load) {
  return (dispatch, getState) => {
    const urls = [
      ...
      //   name: "loutro-old.glb",
      //   size: "0",
      //   url: process.env.PUBLIC_URL + "/demo/loutro-old.glb",
      // },
      {
        name: "tree-green.fbx",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/tree-green.fbx",
      },
      {
        name: "tree-red.fbx",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/tree-red.fbx",
      },
      // {
      //   name: "loutro-new.gltf",
      //   size: "0",
      //   url: process.env.PUBLIC_URL + "/demo/loutro-new.gltf",
      // },
      {
        name: "man-walking.gltf",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/Soldier.glb",
      },
      {
        name: "loutro-old.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/loutro-old.csv",
      },
      {
        name: "loutro-new.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/loutro-new.csv",
      },
      {
        name: "trees-green.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/trees-green.csv",
      },
      {
        name: "trees-red.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/trees-red.csv",
      },
      {
        name: "dtm.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/dtm.csv",
      },
      {
        name: "anime-path-1.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/anime-path-1.csv",
      },
      {
        name: "anime-path-2.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/anime-path-2.csv",
      },
      {
        name: "anime-path-3.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/anime-path-3.csv",
      },
      {
        name: "anime-path-4.csv",
        size: "0",
        url: process.env.PUBLIC_URL + "/demo/anime-path-4.csv",
      },
    ];
    ...
    return load(urls).then(() => {
      ...
      //load trees green
      d = newState.api.models.data.filter(
        (m) => m.name === "tree-green.fbx"
      )[0];
      vector = {
        [d.id]: newState.api.vectors.data.filter(
          (m) => m.name === "trees-green.csv"
        )[0].id,
      };
      ...
      loader.load(
        "../assets/fantasy.jpg",
        function (texture) {
          newState.api.sky.mesh.material = new THREE.MeshBasicMaterial({
            map: texture,
            side: THREE.BackSide,
          });
      ...
      // add ground
      props.loadGround(
        newState.api.vectors.data.filter((m) => m.name === "dtm.csv")[0].id,
        props
      );

      //load loutro old
      // d = newState.api.models.data.filter(
      //   (m) => m.name === "loutro-old.glb"
      // )[0];
      // vector = {
      //   [d.id]: newState.api.vectors.data.filter(
      //     (m) => m.name === "loutro-old.csv"
      //   )[0].id,
      // };
      // rotation = { [d.id]: JSON.stringify({ x: 1.57, y: 0, z: 0 }) };
      ...
      //load loutro new
      // d = newState.api.models.data.filter(
      //   (m) => m.name === "loutro-new.gltf"
      // )[0];
      // vector = {
      //   [d.id]: newState.api.vectors.data.filter(
      //     (m) => m.name === "loutro-new.csv"
      //   )[0].id,
      // };
      // rotation = { [d.id]: JSON.stringify({ x: 1.57, y: 0, z: 0 }) };
      ...
      //load man walking
      d = newState.api.models.data.filter(
        (m) => m.name === "man-walking.gltf"
      )[0];
      vector = {
        [d.id]: newState.api.vectors.data.filter(
          (m) => m.name === "anime-path-1.csv"
        )[0].id,
      };
      rotation = { [d.id]: JSON.stringify({ x: 1.57, y: 0, z: 0 }) };
