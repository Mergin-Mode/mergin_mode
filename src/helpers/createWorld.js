...
export default function createWorld(
  ...
  const camera2 = new THREE.PerspectiveCamera(
    60,
    axesHost.clientWidth / axesHost.clientHeight,
    1,
    10000
  );
  ...
  function onMouseMove(event) {
    ...
    if (intersects.length > 0) {
      ...
      const posX =
        Math.sqrt(
          Math.pow(-50 - -50, 2) + Math.pow(pointer.position.x - -50, 2)
        ) / 100;
      const posY =
        Math.sqrt(
          Math.pow(-50 - -50, 2) + Math.pow(-50 - pointer.position.y, 2)
        ) / 100;
      const posZ = Math.sqrt(
        Math.pow(0 - 0, 2) + Math.pow(0 - pointer.position.z, 2)
      );
      showCoords(posX, posY, posZ);
      window.array = window.array || "";
      window.array += `${pointer.position.x.toFixed(
        4
      )},${pointer.position.y.toFixed(4)},${pointer.position.z.toFixed(4)}\n`;
    }
  }
  ...
  function render() {
    ...
    window.mergin_mode.modelLayer.map((md) => {
      ...
      const activeRowData = window.mergin_mode.vectors.data.filter(
        (v) => v.id == model.vectorId
      )[0].array[0][activeRow];
      ...
      if (
        ...
        if (newSab < Sab) {
          ...
          console.log(
            window.mergin_mode.vectors.data.filter(
              (v) => v.id == model.vectorId
            )[0]
          );
          const newActiveRow = window.mergin_mode.vectors.data.filter(
            (v) => v.id == model.vectorId
          )[0].array[0][activeRow + 1];
          if (!newActiveRow) {
            const startActiveRow = window.mergin_mode.vectors.data.filter(
              (v) => v.id == model.vectorId
            )[0].array[0][0];
            ...
            model.mesh.rotateOnWorldAxis(
              axisZ,
              -(400 - activeRowData[3] + newActiveRow[3]) / 63.6619772367581
            );
          }
        }
